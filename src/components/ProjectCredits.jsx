...
// Standard footer component for credits/details, using dynamic index numbers
export default function ProjectCredits({ items, accentColor, number = "07" }) {
  return (
    <Reveal once={true}>
       <div className="mb-16 flex flex-col md:flex-row gap-10 md:items-start md:justify-between">
          ...
          <div className="flex flex-col md:flex-row gap-12 md:gap-24 md:text-right">
             {items.map((group, index) => (
                 <div key={index}>
                    <h4 className="text-[12px] font-mono uppercase tracking-widest mb-4" style={{ color: accentColor }}>{group.title}</h4>
