...
export default function Home() {
  ...
  // SCROLL SPY: Detects when the user has scrolled past the Hero section
  useEffect(() => {
    const handleScroll = () => {
      const heroHeight = window.innerHeight;
      // Triggers 'projects' state once the user covers 80% of the viewport height
  ...
  // ANIMATION VARIANT: A sophisticated "Blur + Fade + Slide" reveal
  const textVariant = {
  ...
  return (
    <div className="w-full bg-[#0E0E0E] text-[#F4F4F5] overflow-x-hidden relative selection:bg-white selection:text-black cursor-default">
      ...
      <Spotlight /> {/* Interactive cursor glow */}

      {/* HERO HEADER: Centered introduction with staggered text animation */}
      <header className="h-screen w-full flex flex-col justify-center items-center relative px-4 text-center z-10">
        <div className="flex flex-col items-center">
          ...
          {/* PREVIOUS WORK: Subtle branding link with custom underline animation */}
        ...
        {/* SCROLL INDICATOR: Directs user to the projects section */}
      ...
      {/* PROJECT LIST: Uses Reveal component to stagger the entry of ProjectCards */}
